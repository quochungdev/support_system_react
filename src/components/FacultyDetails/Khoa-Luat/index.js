...
const KhoaLuatDetails = () => {
  ...
  const handlePageChange = (pageNumber) => {
    ...
    const filteredArticles = articles.filter(article => article.facultyId.id === 113);
    const filteredFaculties = faculties.filter(faculty => faculty.id === 113);
      
    const currentArticles = filteredArticles.slice(indexOfFirstArticle, indexOfLastArticle);

    return<>
         <h2 className="text-center text-blue-700 font-bold mb-5">GIỚI THIỆU KHOA LUẬT</h2>
    ...
    <Col xs={12} md={12}>
      <h1 className="pb-10 pt-10 text-blue-700 decoration-slate-950" >BÀI VIẾT CỦA KHOA</h1>
...
export default KhoaLuatDetails